...
table = prodb.load_table(cell_types_list[0])
...
if len(cell_types_list)==1:    
    ...
    for cell_type in cell_types_list:
        columns += ["coeffs_"+prot+"_"+cell_type for prot in protocols[cell_type]]
    ...
    title = text_log+"PCA_expectation_"+"_".join(cell_types_list)+".html"
    ...
    if len(cell_types_list)>1:
        ...
        title = text_log+"PCA_expectation_by_cell_types"+"_".join(cell_types_list)+".html"
        fig.write_html(Path(figpath, "similarity_analysis", "PCA", title))
...
columns = []
for cell_type in cell_types_list:
    columns += sum([[col+"_"+cell_type for col in table_columns[cell_type][prot]] for prot in protocols[cell_type]],[])
...
plot(fig)
title = text_log+"PCA_replicates_"+"_".join(cell_types_list)+".html"
fig.write_html(Path(figpath, "similarity_analysis", "PCA", title))
