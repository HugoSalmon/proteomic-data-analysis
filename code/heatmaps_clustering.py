...
prodb = ProteoDataset()
table = prodb.load_table(cell_type, reset=False)
...
cols_test_diff = ["test_diff_"+method2+ "-" + method1 for method1, method2 in list_comparisons]
...
clustered_index, labels, silhouette = plot_hierarchical_clustering(table[cols_discrete], \
                                metric="matching", linkage_method="complete", threshold=1, \
                                distance_matrix=False, criterion="threshold")


indexes = index[np.array(clustered_index)]
...
ax.set_yticklabels(table["Gene name"].iloc[indexes].values)
ax.xaxis.tick_top()
...
fig.savefig(Path(figpath, "heatmaps_clustering", cell_type, cell_type+"_"+test+".png"))
fig2.savefig(Path(figpath, "heatmaps_clustering", cell_type, cell_type+"_"+test+"_"+"short"+".pdf"))
