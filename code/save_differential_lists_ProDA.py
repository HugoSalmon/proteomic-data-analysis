...
for cell_type in cell_types:
    ...
    for k, name in enumerate(masks):
        ...
        coefs_cols = [col for col in table.columns if "coeffs" in col and cond1 in col] + \
            [col for col in table.columns if "coeffs" in col and cond2 in col]
            
        if not os.path.isdir(Path(figpath, "1. lists_differential_proteins_ProDA", cell_type)):
            os.mkdir(Path(figpath, "1. lists_differential_proteins_ProDA", cell_type))
            
            
        to_save = subtable[["ENTREZ_GENE_ID", "GENE_NAME", "UNIPROT_ACCESSION", col_pval, col_FC]+LFQI_cols+coefs_cols]
        to_save = to_save.sort_values(by=col_FC, ascending=False)
        
        header = ["Gene_ID", "Gene_name", "Protein_ID", "pval", "log2_Fold change"]+ \
            ["log2_"+col for col in LFQI_cols]+[col.replace("coeffs","proDA_expected_log2len_intensity") for col in coefs_cols]
    
        to_save.to_csv(Path(figpath, "1. lists_differential_proteins_ProDA", cell_type, "list_diff_"+cell_type+"_"+test+"_"+name+".csv"), 
        ...
        fig.write_html(Path(figpath, "1. lists_differential_proteins_ProDA", cell_type, "volcano_plot_"+cell_type+"_"+test+"_"+name+".html"))
    ...
    if plot_enrichments:
        ...
        if not os.path.isdir(Path(figpath, "2. enrichments_differential_proteins_ProDA", cell_type)):
            os.mkdir(Path(figpath, "2. enrichments_differential_proteins_ProDA", cell_type))
    
        
        save_enrichments_results(prodb, table, masks, index_, names, names_to_plot=names, species=species, savepath=Path(figpath, "2. enrichments_differential_proteins_ProDA", cell_type), test=test)
