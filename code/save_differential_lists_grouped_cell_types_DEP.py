...
#################### For STRING DB
masks, names, index_, cols = get_masks_test_diff_only_pair(table, ["THP1", "Fibro", "MSCH", "MSCSmapH"], test="DEP")
...
for k, name in enumerate(masks):
    ...
    for cell_type in cell_types.split("-"):
        ...
        pval_cols.append("pval_DEP_"+method2+"-"+method1+"_"+cell_type)
        
        FC_cols.append("FC_DEP_"+method2+"-"+method1+"_"+cell_type)
        

    
    # subtable_log.to_csv(Path(savepath, "FoldChange(log)_"+cell_type+"_"+name+".txt"), header=None, index=None, sep=' ')

    subtable[["ENTREZ_GENE_ID", "GENE_NAME"]+pval_cols+FC_cols].to_csv(Path(figpath, "3. lists_differential_proteins_DEP_grouped_cell_types", "list_diff_"+name+".txt"), header=["Gene_ID", "Gene_name"]+pval_cols+FC_cols, index=None, sep=' ')
...
if plot_enrichments:
    ...
    save_enrichments_results(prodb, table, masks, index_, names, names_to_plot=names, species=species, savepath=Path(figpath, "4. enrichments_differential_proteins_DEP_grouped_cell_types"), test="DEP")
