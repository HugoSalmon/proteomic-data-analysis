...
for cell_type in cell_types:
    ...
    #################### For STRING DB
    masks, names, index_ = get_masks_on(table, [cell_type])
    ...
    for k, name in enumerate(masks):
        ...
        condition = names[k].replace("present_","")
                
        LFQI_cols = [col for col in subtable.columns if "LFQI" in col and condition in col]
    
        if not os.path.isdir(Path(figpath, "5. lists_present_proteins_in_at_least_one_replicate", cell_type)):
            os.mkdir(Path(figpath, "5. lists_present_proteins_in_at_least_one_replicate", cell_type))
    
        subtable[["ENTREZ_GENE_ID", "GENE_NAME", "UNIPROT_ACCESSION"]+ LFQI_cols].to_csv(Path(figpath, "5. lists_present_proteins_in_at_least_one_replicate", cell_type, "list_present_proteins_"+cell_type+"_"+name+".csv"), header=["Gene_ID", "Gene_name", "Protein_ID"]+LFQI_cols, index=None)
    ...
    if plot_enrichments:
        ...
        if not os.path.isdir(Path(figpath, "6. enrichments_present_proteins_in_at_least_one_replicate", cell_type)):
            os.mkdir(Path(figpath, "6. enrichments_present_proteins_in_at_least_one_replicate", cell_type))
    
        
        save_enrichments_results(prodb, table, masks, index_, names, names_to_plot=names, species=species, savepath=Path(figpath, "6. enrichments_present_proteins_in_at_least_one_replicate", cell_type))
